...
const starts = async (arul = new WAConnection()) => {
    ...
    arul.version =  [2, 2142, 12];
    arul.browserDescription = Browsers.ubuntu('Safari')
    arul.on('qr', () => {
   ...
   console.log(color('SUPPORT by https://pecundang.herokuapp.com', 'cyan'))
    ...
    fs.existsSync(`./${ses}.json`) && arul.loadAuthInfo(`./${ses}.json`)
    arul.on('connecting', () => {
    ...
    await arul.connect({timeoutMs: 30*1000})
        fs.writeFileSync(`./${ses}.json`, JSON.stringify(arul.base64EncodedAuthInfo(), null, '\t'))
	console.log(arul.user.jid.split('@')[0])
	
	arul.on('chat-update', async (msg) => {
		require('./msgHndlr')(arul, msg)
	})
